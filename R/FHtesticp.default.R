...
function(L, R, group, rho=0, lambda=0,alternative, permcontrol=permControl(), icFIT=NULL,initfit=NULL,icontrol=icfitControl(),exact=NULL,method=NULL,methodRule=methodRuleIC1,Lin=NULL,Rin=NULL,...){
	...
	if (k>2) icp <- ictest(L,R,group, scores = "general",dqfunc=function(x){x*pbeta(1-x,lambda+1,rho+10^-15)*gamma(lambda+1)*gamma(rho+10^-15)/gamma(lambda+rho+1)}, alternative=icalternative, mcontrol=permcontrol,icFIT=icFIT,initfit=initfit,icontrol=icontrol,exact=exact,method=method,methodRule=methodRule,Lin=Lin,Rin=Rin,...)
	else{
		ord <- order(group)
		if(length(Lin)>1) Lin <- Lin[ord]
		if(length(Rin)>1) Rin <- Rin[ord] 
		icp <- ictest(L[ord],R[ord],group[ord], scores = "general",dqfunc=function(x){x*pbeta(1-x,lambda+1,rho+10^-15)*gamma(lambda+1)*gamma(rho+10^-15)/gamma(lambda+rho+1)}, alternative=icalternative, mcontrol=permcontrol,icFIT=icFIT,initfit=initfit,icontrol=icontrol,exact=exact,method=method,methodRule=methodRule,Lin=Lin,Rin=Rin,...)		
